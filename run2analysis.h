...
//
   ...
   //Float_t         pfmht;
   Int_t           njets;
   Float_t         jet_pt[100];   //[njets]
   Float_t         jet_eta[100];   //[njets]
   Float_t         jet_phi[100];   //[njets]
   Float_t         jet_E[100];   //[njets]
   Float_t         jet_m[100];   //[njets]
   Float_t         jet_id[100];   //[njets]
   Float_t         jet_et[100];   //[njets]
   ...
   virtual void     Loop(int year, TString Letter, bool dataFlag);
   virtual Bool_t   Notify();
   virtual void     Show(Long64_t entry = -1);
   double getMassSpecial(float ih, float p, float K, float C, float N);
...
//   float FMIP(const vector<float>& charge, const<vector>& path,  float thre);
   float FMIP(std::vector <float> charge, std::vector <float> path, float SFactor,  float thre);
   ...
   void loadSFPixelTamas();
   float GetSFPixelTamas(int subdetid_, UInt_t detid_, int year, int run);
   int icalibL1;
   int icalibL1_2017;
   int icalibL1_2018;
   int icalibL2;
